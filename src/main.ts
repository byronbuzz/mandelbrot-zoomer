...
fn dsAdd(a: vec2f, b: vec2f) -> vec2f { let s=a.x+b.x; let v=s-a.x; let e=(a.x-(s-v))+(b.x-v)+a.y+b.y; let hi=s+e; return vec2f(hi,e-(hi-s)); }
fn dsSub(a: vec2f, b: vec2f) -> vec2f { return dsAdd(a,vec2f(-b.x,-b.y)); }
fn dsMul(a: vec2f, b: vec2f) -> vec2f { let product=a.x*b.x; let error=fma(a.x,b.x,-product)+a.x*b.y+a.y*b.x+a.y*b.y; let hi=product+error; return vec2f(hi,error-(hi-product)); }
fn dsScale(a: vec2f, b: f32) -> vec2f { let product=a.x*b; let error=fma(a.x,b,-product)+a.y*b; let hi=product+error; return vec2f(hi,error-(hi-product)); }
fn paletteColour(t:f32)->vec3f { return .5+.5*cos(6.28318*(vec3f(t)+vec3f(0.0,.12,.24)+p.phase)); }
fn writeResult(id:vec2u,escaped:bool,iteration:u32,radius:f32){ if(!escaped){textureStore(outTex,vec2i(id),vec4f(0,0,0,1));return;} let smoothValue=f32(iteration)+1.0-log2(log2(sqrt(radius))); textureStore(outTex,vec2i(id),vec4f(paletteColour(fract(.018*smoothValue)),1)); }
@compute @workgroup_size(8,8)
fn main(@builtin(global_invocation_id) gid:vec3u){
 let id=gid.xy; if(id.x>=p.width||id.y>=p.height){return;}
 let uv=(vec2f(id)+.5)/vec2f(f32(p.width),f32(p.height)); let ox=(uv.x-.5)*p.aspect; let oy=uv.y-.5;
 if(p.mode==0u){
  let c=vec2f(p.centerX.x,p.centerY.x)+vec2f(ox,oy)*p.scale.x; let q=(c.x-.25)*(c.x-.25)+c.y*c.y;
  if(q*(q+c.x-.25)<=.25*c.y*c.y||(c.x+1.)*(c.x+1.)+c.y*c.y<=.0625){textureStore(outTex,vec2i(id),vec4f(0,0,0,1));return;}
  var z=vec2f(0.); var iteration=0u; var radius=0.0;
  loop{radius=dot(z,z);if(iteration>=p.iterations||radius>256.){break;}z=vec2f(z.x*z.x-z.y*z.y,2.*z.x*z.y)+c;iteration++;}
  writeResult(id,iteration<p.iterations,iteration,radius);return;
 }
 let cx=dsAdd(p.centerX,dsScale(p.scale,ox)); let cy=dsAdd(p.centerY,dsScale(p.scale,oy)); let cxApprox=cx.x+cx.y; let cyApprox=cy.x+cy.y; let q=(cxApprox-.25)*(cxApprox-.25)+cyApprox*cyApprox;
 if(q*(q+cxApprox-.25)<=.25*cyApprox*cyApprox||(cxApprox+1.)*(cxApprox+1.)+cyApprox*cyApprox<=.0625){textureStore(outTex,vec2i(id),vec4f(0,0,0,1));return;}
 var zx=vec2f(0.); var zy=vec2f(0.); var iteration=0u; var radius=0.0;
 loop{radius=zx.x*zx.x+zy.x*zy.x;if(iteration>=p.iterations||radius>256.){break;}let zx2=dsMul(zx,zx);let zy2=dsMul(zy,zy);let zxy=dsMul(zx,zy);zx=dsAdd(dsSub(zx2,zy2),cx);zy=dsAdd(dsScale(zxy,2.0),cy);iteration++;}
...
if (shaderErrors.length) { const first=shaderErrors[0]; const detail=`WGSL line ${first.lineNum}: ${first.message}`; status.textContent=detail; throw new Error(detail); }
...
async function renderFrame(){
 ...
 const w=Math.max(1,Math.floor(canvas.clientWidth*devicePixelRatio*quality.resolution)),h=Math.max(1,Math.floor(canvas.clientHeight*devicePixelRatio*quality.resolution));canvas.width=w;canvas.height=h;
 const deepMode=3/scale>=DOUBLE_FLOAT_THRESHOLD,[cxHi,cxLo]=split(centerX),[cyHi,cyLo]=split(centerY),[scaleHi,scaleLo]=split(scale);
 const texture=device.createTexture({size:[w,h],format:canvasFormat,usage:GPUTextureUsage.STORAGE_BINDING|GPUTextureUsage.COPY_SRC});const data=new ArrayBuffer(48),f=new Float32Array(data),u=new Uint32Array(data);
 f[0]=cxHi;f[1]=cxLo;f[2]=cyHi;f[3]=cyLo;f[4]=scaleHi;f[5]=scaleLo;f[6]=w/h;u[7]=quality.iterations;f[8]=Number(palette.value);u[9]=w;u[10]=h;u[11]=deepMode?1:0;device.queue.writeBuffer(params,0,data);
 const group=device.createBindGroup({layout:pipeline.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:params}},{binding:1,resource:texture.createView()}]});const encoder=device.createCommandEncoder(),pass=encoder.beginComputePass();pass.setPipeline(pipeline);pass.setBindGroup(0,group);pass.dispatchWorkgroups(Math.ceil(w/8),Math.ceil(h/8));pass.end();encoder.copyTextureToTexture({texture},{texture:gpuContext.getCurrentTexture()},{width:w,height:h});
 const started=performance.now();device.queue.submit([encoder.finish()]);await device.queue.onSubmittedWorkDone();const frameMs=Math.max(.1,performance.now()-started);texture.destroy();smoothedFrameMs=smoothedFrameMs*.78+frameMs*.22;fpsValue=1000/smoothedFrameMs;if(activeStage==='interactive')updateController(smoothedFrameMs);frameInFlight=false;updateReadouts();
 if(!isInteractive()&&renderStage==='refining'){if(refineStep<2)refineTimer=window.setTimeout(()=>{refineStep++;requestRender();},REFINE_DELAY_MS);else refineTimer=window.setTimeout(()=>{renderStage='full-quality';requestRender();},REFINE_DELAY_MS);}if(renderRequested)schedulePump();
}
function schedulePump(){if(pumpScheduled)return;pumpScheduled=true;requestAnimationFrame(()=>{pumpScheduled=false;if(!frameInFlight&&renderRequested)void renderFrame();});}
function requestRender(){renderRequested=true;schedulePump();}
...
canvas.addEventListener('pointermove',e=>{pointer(e);if(!panning||e.pointerId!==panPointer)return;const r=canvas.getBoundingClientRect();centerX-=(e.clientX-panX)/r.width*(r.width/r.height)*scale;centerY-=(e.clientY-panY)/r.height*scale;panX=e.clientX;panY=e.clientY;markInteraction();requestRender();});
canvas.addEventListener('pointerdown',e=>{pointer(e);canvas.setPointerCapture(e.pointerId);if(e.button===1){panning=true;panPointer=e.pointerId;panX=e.clientX;panY=e.clientY;direction=0;}else direction=e.button===2?-1:1;markInteraction();requestRender();e.preventDefault();});
