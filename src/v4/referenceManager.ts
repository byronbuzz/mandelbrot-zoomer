...
export class ReferenceManager {
  private readonly worker = new Worker(new URL('./referenceWorker.ts', import.meta.url), { type: 'module' });
  ...
  request(camera: CameraSnapshot, iterations: number, purpose: ReferencePurpose, aspect: number): boolean {
    const key = `${camera.generation}:${iterations}:${purpose}`;
    if (this.pendingKeys.has(key)) return false;
    const id = ++this.nextRequestId;
    const candidates = CANDIDATE_OFFSETS.map(([dx, dy]) => ({
    ...
    this.pending.set(id, { cameraGeneration: camera.generation, purpose, requestedIterations: iterations, key });
