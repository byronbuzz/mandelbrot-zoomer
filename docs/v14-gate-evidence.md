...
The required invariant is monotonic display evidence after transforming the previous display into the new target view: once a target complex coordinate has a valid displayed source, later frames may replace it only with semantically stronger evidence. A binary validity bit or undifferentiated confidence scalar is insufficient. Rolling history must carry at least source kind, semantic class, evidence depth, projected source footprint, view revision and valid atlas lease. Semantic class must distinguish invalid/failure, escaped with escape iteration, analytically proven interior, provisional cap with tested frontier, and final cap at the selected target. New current-view escape or analytic-interior results replace estimates immediately. A provisional cap may fill an invalid hole, but may not replace a prior escaped, analytic or final sample. One provisional estimate may replace another only when its iteration evidence, source footprint and numerical reliability are all no worse; incomparable candidates preserve the existing display sample. A final cap may replace an estimate only when it reaches the selected target. The evidence is retained as `outputs/v14-evidence/live-zoom-in-5000-black-flash-depth5.png` and `live-zoom-in-5000-iteration-regression-depth1.png`; these two screenshots are qualitative failing evidence and do not yet have exact-camera, frame-synchronized JSON records.
