...
The required invariant is monotonic display evidence after transforming the previous display into the new target view: once a target complex coordinate has a valid displayed source, later frames may replace it only with semantically stronger evidence. A binary validity bit or undifferentiated confidence scalar is insufficient. Rolling history must carry at least source kind, semantic class, evidence depth, projected source footprint, view revision and valid atlas lease. Semantic class must distinguish invalid/failure, escaped with escape iteration, analytic-interior result, provisional cap with tested frontier, and final cap at the selected target. New current-view escape or analytic-interior results replace estimates immediately. A provisional cap may fill an invalid hole, but may not replace a prior escaped, analytic or final sample. Between compatible capped estimates, a candidate may replace the display only when its tested frontier and projected footprint are both no worse; incomparable candidates preserve the existing display. The presenter does not rank direct, expansion, perturbation or repair modes and must not feed provenance back into numerical scheduling. A final cap means only “not observed escaping through the currently selected target”; reprojection into another view demotes it to an estimate. The evidence is retained as `outputs/v14-evidence/live-zoom-in-5000-black-flash-depth5.png` and `live-zoom-in-5000-iteration-regression-depth1.png`; these two screenshots are qualitative failing evidence and do not yet have exact-camera, frame-synchronized JSON records.

The UI's rolling readout indicated 60 Hz and zero WebGPU validation errors during the traces; it was not frame-synchronized proof that each flash met cadence. Validation-clean submission is not a coverage guarantee. The root architectural defect is that the presenter retains only one full-screen history view. During zoom-out that source maps to a small rectangle inside the newly exposed viewport; the reprojection shader clears all source-UV misses to black, while the accepted atlas has too few retained coarser/outside tiles to cover the remainder. Subsequent 128x128 tile admission causes the visible block in-fill and jumps.

This is a Release 1.4 presentation blocker. The milestone must remain draft until a rolling display head with explicit validity and semantic per-pixel evidence prevents regression in both directions, a bounded multi-scale/overscanned history or equivalent retained coarse-coverage layer keeps the full target viewport continuously covered during zoom-out, and executable pre-settle gates pass deterministic zoom-in and the recorded `10^8 -> 10^2` zoom-out trace. Zoom-out evidence is stored in `outputs/v14-evidence/live-zoom-out-5000-order-{7..2}.{png,json}` and `live-zoom-out-5000-summary.json`.
...
Release checklist status: the behavioral pre-settle full-coverage and monotonic-evidence gates are **FAILED / PENDING**. Release 1.4 must not advance or deploy from this draft PR until frame-synchronized validity/provenance gates pass both presenters in both navigation directions.
...
6. Empty current-tile packets skipped presentation entirely. The atlas path now presents history with a zero-instance overlay. This enables history-only frames but does not by itself establish full coverage or non-regressing display evidence.
